...
Versione: **18.1** · Lingue supportate nativamente (9): Polski, Deutsch, English, Español, Suomi, Français, Íslenska, Italiano, Русский.
...
### 2. Opowieści (Ctrl+2, secondo modalità principale dalla versione 15.0)

Giochi di testo interattivi condotti dall'AI nel ruolo di motore narrativo. A differenza della Regia (dove generi un audiolibro finito), le Opowieści sono una trama dinamica turno per turno:
...
* **Modalità Minore Male:** come le Scelte, ma ogni opzione è sfavorevole moralmente, fisicamente o strategicamente. Dalla versione 15.2, un'ulteriore "fiala" — opzione di salvataggio disperato numerata ZERO riutilizzabile, i cui effetti sono pseudo-casuali (60% dannosi / 30% che alterano la percezione / 10% raramente vantaggiosi, distribuzione forzata da Python, LLM non può inventare un effetto salvifico).
...
* **Un modello AI per tutte le modalità:** dalla versione 18.1 tutte le modalità Opowieści utilizzano Anthropic Claude Sonnet 4.6 — un modello più potente che aderisce rigorosamente alle regole del mondo (cruciale soprattutto nella modalità Minore Male, dove ogni opzione deve essere realmente sfavorevole).
...
L'interfaccia GUI, la documentazione (`docs/manual.<iso>.txt`) e la maggior parte dei messaggi di sistema sono disponibili nativamente in ciascuna delle lingue supportate. I prompt di sistema AI nelle modalità Regista e Racconti sono scritti nelle lingue di destinazione (manualmente, non auto-tradotti — vedi `dictionaries/<kod>/rezyser/` e `dictionaries/<kod>/opowiesci/`).


## Architettura AI e modelli utilizzati

L'applicazione distribuisce intelligentemente i compiti tra due fornitori di API, ottimizzando la qualità della prosa, i costi e la velocità:

* **Anthropic Claude Sonnet 4.6:** Motore di tutta la narrazione creativa. Responsabile della regia degli script, della scrittura di prosa tradizionale (Audiolibro), del Brainstorming e di TUTTE le modalità di Racconto (Scelte, Minor Male, Libero) insieme alla generazione di riassunti e intermezzi Cinematic. La migrazione della narrazione su Claude (Regista in v18.0, Racconti in v18.1) è risultata dalla comprovata superiorità empirica nel mantenere le regole del mondo e nell'evitare cliché.
* **gpt-4o (OpenAI):** Traduzioni avanzate con mantenimento del contesto multi-blocco (Poliglotta). Migrazione su Anthropic pianificata nella prossima versione.
* **gpt-4o-mini (OpenAI):** Modello ausiliario veloce e leggero per micro-compiti: assegnazione iterativa di titoli letterari ai capitoli generati ed estrazione di codici ISO della lingua.
