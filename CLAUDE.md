...
7. KLUCZE API DOSTĘPNE (DUAL-PROVIDER od 18.0): `golden_key.env` w roocie repo jest gitignorowany, ale fizycznie obecny u dewelopera, i trzyma DWA klucze. **`OPENAI_API_KEY`** — dev-tooling tłumaczeń (`buduj_wielojezyczne_ui.py`, `buduj_wielojezyczne_docs.py`, `tlumacz_ai.py`) + runtime: postprodukcja tytułów i mikro-call kodu języka (`gpt-4o-mini`) + Opowieści/Poliglota. **`ANTHROPIC_API_KEY`** (`sk-ant-`) — runtime narracji Reżysera (audiobook/skrypt/burza) na Claude Sonnet 4.6 (migracja 18.0 — GPT-4o niezdatny do prozy; pełny kontekst + decyzje w [[reguly_architektury]]). Agent **może i powinien** odpalać skrypty tłumaczeń bezpośrednio (ładują klucz przez `python-dotenv`) — nie przekazuj tego kroku userowi „bo nie masz API". Stop tylko gdy skrypt zwróci „❌ Brak prawidłowego OPENAI_API_KEY" lub HTTP 401/429 — wtedy poproś usera o uzupełnienie. Po auto-tłumaczeniach ZAWSZE manualny review halucynacji — generyczne sanity checki + szczegółowe hotspoty per szyfr/sekcja w [[reguly_tlumaczen]].
...
5. **Bezpieczniejsza kolejność (od v17.8.0):** build EXE lokalnie (woła `generuj()` → wyłapie nieświeże docs po late-changes `ui.yaml`/`app.wersja`) → `git status` → jeśli niezcommitowane `docs/*.txt`, **amend release commit ZANIM pushniesz** → dopiero `git push origin main`. Po pushu poprawka = osobny fixup commit (zakaz force-push na main). Detale → [[reguly_git_workflow]].
