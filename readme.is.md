...
Zestak af flytjanlegum tólum knúnum af gervigreind til sjálfvirkrar ritunar, skipulagningar, sniðmáts og þýðingar á umfangsmiklum handritum og til að stýra gagnvirkum textaleikjum. Verkefnið er innfæddur skjáborðsforrit (wxPython) hannað frá grunni með fullu aðgengi fyrir skjálesara (NVDA, VoiceOver) og samhæfni við faglega talgervla (TTS). Það virkar án vafra og án staðbundins netþjóns — keyrir sem venjulegur gluggi forrits.

Útgáfa: **15.1** · Stutt tungumál innfædd (9): Polski, Deutsch, English, Español, Suomi, Français, Íslenska, Italiano, Русский.
...
- Enska (Samantha/Mark í Vocalizer, Zira/Hazel í OneCore)
- Finnska (Satu/Mikko í Vocalizer, Heidi í OneCore)
- Ítalska (Alice/Luca í Vocalizer, Elsa í OneCore)
- Pólska (Ewa í Vocalizer, Paulina í OneCore)

## Dulmál: Textaumhverfa

Textaumhverfa dulmálið breytir setningum með því að snúa þeim við. Áður en setning er snúið við, stækkar handritið ákveðnar skammstafanir til að koma í veg fyrir hljóðfræðilegt bull. Til dæmis, ".td" verður ".dæt" ef það er ekki stækkað fyrst. Skammstafanir sem eru stækkaðar eru:
...
Samkvæmt rannsóknum við Cambridge háskóla skiptir ekki máli í hvaða röð stafirnir í orði eru, það eina sem skiptir máli er að fyrsti og síðasti stafurinn séu á réttum stað.

"Samkvæt rnasóknum við Cmabrigde háksóla skiptir ekki máli í hvaða röð stfirnri í oðri eru, það eina sem skiptir máli er að fyrsti og síðasti stafurinn séu á réttum stað."
...
1. Sæktu nýjustu útgáfuna úr **Útgáfur** flipanum (pakki merktur sem *Nýjasta*). Tvær útgáfur eru í boði:
   * **Installer EXE** — setur upp í Program Files (eða valinn möppu), býr til flýtileiðir í Start-valmyndinni og á skjáborðinu. Eftir uppsetningu opnar valfrjálst notendahandbók í sjálfgefnum .txt lesara.
   * **Portable ZIP** — pakkaðu út í hvaða möppu sem er, krefst ekki stjórnandaaðgangs. Eftir útpökkun skaltu keyra `run.bat`.
...
* `docs/opowiesci.<iso>.txt` — handbók fyrir sögumóða (gagnvirkir textaleikir).
