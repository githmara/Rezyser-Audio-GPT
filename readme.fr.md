...
Aplikacja obsługuje wiele akcentów dla syntezatora mowy, co pozwala na dostosowanie brzmienia do potrzeb użytkownika. Dostępne akcenty to:
...
## Szyfr: Odwracacz tekstu

Ten szyfr odwraca kolejność liter w zdaniu, ale najpierw rozwija niektóre skróty, aby uniknąć fonetycznych nonsensów. Obsługiwane skróty to:
...
Bez rozwinięcia, skróty z kropkami odwróciłyby się w fonetyczny nonsens (przykład: ".xe").

## Szyfr: Typoglycémie

Ce chiffre repose sur l'idée que les gens peuvent lire des mots même si les lettres au milieu sont mélangées, tant que la première et la dernière lettre restent à leur place. Voici un exemple :

"Seoln une étdue de l'Unviesirté de Cmabrigde, l'odrre des ltteers n'a pas d'imporatnce, ce qui cmopmte c'est que la pmeirère et la draineère soient à la bnnoe pclae."

## Pliki konfiguracyjne

Aplikacja używa plików YAML do konfigurowania różnych aspektów działania. Oto lista dostępnych plików:
...
1. Téléchargez la dernière version depuis l'onglet **Releases** (paquet marqué comme *Latest*). Deux formats sont disponibles :
   * **Installer EXE** — installe dans Program Files (ou dans le dossier choisi), crée des raccourcis dans le Menu Démarrer et sur le bureau. À la fin de l'installation, ouvre éventuellement le manuel d'utilisation dans le gestionnaire par défaut de fichiers .txt.
   * **Portable ZIP** — extrayez dans n'importe quel dossier, ne nécessite pas de droits administratifs. Après extraction, lancez `run.bat`.
...
* `docs/opowiesci.<iso>.txt` — le manuel du mode Histoires (jeux textuels interactifs).
